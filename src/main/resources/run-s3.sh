...
hdfs dfs -ls ${DFS}/user/$HADOOP_USER
if [ $? != 0 ] ;then
  hdfs dfs -mkdir ${DFS}/user/"$HADOOP_USER"
fi

hdfs dfs -ls ${DFS}/user/$HADOOP_USER/${HDFS_DIR}
if [ $? != 0 ] ;then
  hdfs dfs -mkdir ${DFS}/user/$HADOOP_USER/${HDFS_DIR}
fi
...
hdfs dfs -rm -r -skipTrash ${DFS}/user/"$HADOOP_USER"/${HDFS_DIR}/*
...
# DataGen
mkdir -p ./logs
(time spark-submit --class ${TERAGEN_CLASS_NAME} --deploy-mode ${SPARK_DEPLOY_MODE} --master ${SPARK_MASTER_URL} --conf "spark.driver.memory=${SPARK_DRIVER_MEMORY}" --conf "spark.executor.memory=${SPARK_EXECUTOR_MEMORY}" ${SPARK_TERASORT_JAR} ${hssize} ${DFS}/user/"${HADOOP_USER}"/"${HDFS_DIR}"/HSsort-input ) 2>&1 | (tee ./logs/HSgen-time-run${i}.txt)
result=$?
...
echo "" | tee -a ./result-"$prefix".log
hdfs dfs -ls ${DFS}/user/"$HADOOP_USER"/"${HDFS_DIR}"/HSsort-input/* | tee -a ./result-"$prefix".log
echo "" | tee -a ./result-"$prefix".log
...
echo "" | tee -a ./result-"$prefix".log
hdfs dfs -ls ${DFS}/user/"$HADOOP_USER"/"${HDFS_DIR}"/HSsort-output/* | tee -a ./result-"$prefix".log
echo "" | tee -a ./result-"$prefix".log
...
(time spark-submit --class ${TERAVAL_CLASS_NAME} --deploy-mode ${SPARK_DEPLOY_MODE} --master ${SPARK_MASTER_URL} --conf "spark.driver.memory=${SPARK_DRIVER_MEMORY}" --conf "spark.executor.memory=${SPARK_EXECUTOR_MEMORY}" ${SPARK_TERASORT_JAR} ${DFS}/user/"${HADOOP_USER}"/"${HDFS_DIR}"/HSsort-output) 2>&1 | (tee ./logs/HSvalidate-time-run${i}.txt)
result=$?
